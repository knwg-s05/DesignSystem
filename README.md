...
複数のアプリから参照する、表示の語彙をまとめた Swift パッケージ。
...
**参照するアプリは非公開のまま。**このリポジトリを公開しているのは、アプリ側の CI が
チェックアウトするときに資格情報を要らなくするため。ここには色・余白・書体・スタイルの
定義しか置かないので、公開してもアプリの内容は出ない。

⚠️ **アプリの名前をここへ書かない。**未公開のアプリの存在と名前が、公開リポジトリの
履歴に残る形で外へ出る。README・コメント・型名・テストのいずれにも書かない。
...
題材の語が入る `〜Card` は入らない。ここを緩めると 2 本目のアプリが 1 本目の語彙を
引きずり、共有する意味が無くなる。
...
このパッケージを取り込んでいるアプリを開いていると、`Package.swift` を
